...
# #set device
# device = available_devices(threshold=10000,n_devices=8)
...
def main(args):
    ...
    if args.resmue:
        ...
        resume_step = int(resume_step.cpu())
...
if __name__ == "__main__":
    dataset = 'x_y1' #cat2dog/wild2dog/male2female/multi_afhq(mutli-domain)
    #defalut args
    args = create_argparser().parse_args()
    args.dataset = dataset
    dir = os.path.join('', args.dataset, '10X_dse')
    now = datetime.datetime.now().strftime("%Y-%m-%d-%H-%M-%S")
    dir = os.path.join(dir,now)
